...
export function InfiniteMarquee({ items = DEFAULT_ITEMS, reverse = false }: InfiniteMarqueeProps) {
  ...
  return (
    <View className="w-full overflow-hidden border-y border-border bg-surface/50 py-3.5 select-none">
      <div className={reverse ? 'animate-marquee-reverse' : 'animate-marquee'}>
        {content.map((item, index) => (
          <div key={index} className="flex items-center gap-6 px-6">
            <span className="text-text font-black text-xs sm:text-sm md:text-base tracking-[0.2em] uppercase font-mono">
              {item}
            </span>
            <span className="text-primary text-sm sm:text-base font-black">✦</span>
          </div>
        ))}
      </div>
    </View>
