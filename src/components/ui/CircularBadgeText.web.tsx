...
export function CircularBadgeText({
  ...
  return (
    <View
      className="relative items-center justify-center select-none"
      style={{ width: size, height: size }}
    >
      {/* Rotating Circular Text SVG on Web */}
      <svg
      ...
      >
        {icon === 'arrow' ? (
          <ArrowUpRight size={size * 0.2} color={colors.primary} />
        ) : (
          <Sparkles size={size * 0.2} color={colors.primary} />
        )}
      </View>
    </View>
  );
